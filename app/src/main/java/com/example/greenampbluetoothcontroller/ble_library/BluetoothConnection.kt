...
class BluetoothConnection internal constructor(private val device: BluetoothDevice) {
    ...
    private var activeObservers =
        hashMapOf<String, OnCharacteristicValueChangeCallback<ByteArray>>()
    private var enqueuedReadCallbacks =
        hashMapOf<String, OnCharacteristicValueReadCallback<ByteArray>>()
     ...
     **/
    val services
        get() = this.genericAttributeProfile?.services?.map { BluetoothService(it) } ?: listOf()
    ...
    fun readAsync(characteristic: String, charset: Charset, callback: (String?) -> Unit) =
        readAsync(characteristic) { callback(it?.let { String(it, charset) }) }
    // endregion

    // region Value observation related methods
    private fun legacyObserve(
        owner: LifecycleOwner,
        characteristic: BluetoothCharacteristic,
        callback: OnCharacteristicValueChangeCallback<ByteArray>,
        interval: Long
    ) {
        this.coroutineScope?.launch {
            ...
            val key = characteristic.uuid.toString().lowercase()
            var startTime: Long
            while (owner.lifecycle.currentState != Lifecycle.State.DESTROYED && activeObservers.containsKey(
                    key
                )
            ) {
    ...
    fun observeString(
        ...
        callback: OnCharacteristicValueChangeCallback<String>
    ) {

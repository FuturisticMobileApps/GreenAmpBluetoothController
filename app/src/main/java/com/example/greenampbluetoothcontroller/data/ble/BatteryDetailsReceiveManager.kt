package com.example.greenampbluetoothcontroller.data.ble
...
import com.example.greenampbluetoothcontroller.data.interactors.BatteryDetailsReceiver
...
@Singleton
class BatteryDetailsReceiveManager @Inject constructor(
    private val bluetoothAdapter: BluetoothAdapter,
    private  val context : Context,
) : BatteryDetailsReceiver {
    ...
    override val data: MutableSharedFlow<Resource<BatteryDetails>> = MutableSharedFlow()
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if(status == BluetoothGatt.GATT_SUCCESS){
                if(newState == BluetoothProfile.STATE_CONNECTED){
                    ...
                    this@BatteryDetailsReceiveManager.gatt = gatt
                } else if(newState == BluetoothProfile.STATE_DISCONNECTED){
                    coroutineScope.launch {
                        data.emit(Resource.Success(data = BatteryDetails(0f,0f, ConnectionState.Disconnected)))
                    }
                    gatt.close()
                }
            }else{
            ...
            with(characteristic){
                when(uuid){
                    ...
                    UUID.fromString(TEMP_HUMIDITY_CHARACTERISTICS_UUID) -> {
                        ...
                        val tempHumidityResult = BatteryDetails(
